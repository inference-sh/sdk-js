...
describe('createActions', () => {
  ...
  describe('delta stream listener', () => {
    ...
    it('should reset the legacy shared accumulator when a new assistant message starts', async () => {
      const { ctx, dispatch } = createTestContext();
      const { internalActions } = createActions(ctx);
      ...
      const lastPayload = lastDeltaTokenPayload(dispatch);
      expect(lastPayload?.messageId).toBeUndefined();
      expect(lastPayload?.output.response).toBe('');
      expect(lastPayload?.output.tool_calls?.[0]?.function?.arguments).toBe('{"q"');
    });

    it('should not reset the legacy accumulator when an existing assistant message is updated', async () => {
      ...
      const lastPayload = lastDeltaTokenPayload(dispatch);
      expect(lastPayload?.output.response).toBe('Hello');
    });
